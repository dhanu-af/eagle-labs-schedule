...
/** Renders nothing when there's nothing pending -- this card is meant to read as
 * an urgent alert, not a permanent dashboard fixture, and it's already scoped to
 * exactly one employee (the caller only ever passes in this session's own
 * pending/in-progress requests via listMyPendingTaskRequests()). */
export default function MyTaskRequestsCard({ requests }: { requests: MyTaskRequestRow[] }) {
  ...
  return (
    <div className="card-shadow rounded-2xl border border-warning/40 bg-warning/5 p-5">
